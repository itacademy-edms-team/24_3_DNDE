...
function OtherResource({
  id,
  character,
  onCharacterOtherResourceTotalChange,
  onCharacterOtherResourceCurrentChange,
  onCharacterOtherResourceNameChange,
}) {
  ...
  const handleTotalChange = (event) => {
    ...
    setTotal(newValue);
    onCharacterOtherResourceTotalChange({ total: newValue });
  };

  const handleCurrentChange = (event) => {
    const newValue = event.target.value;
    setCurrent(newValue);
    onCharacterOtherResourceCurrentChange({ current: newValue });
  };

  const handleNameChange = (event) => {
    const newValue = event.target.value;
    setName(newValue);
    onCharacterOtherResourceNameChange({ name: newValue });
  };
  ...
  return (
    <Card className="border-0 p-0 m-0">
      ...
      <Card.ImgOverlay className="p-0 m-0 d-flex flex-column justify-content-between">
        <Form.Group
          controlId={`otherResouce${id}TotalAmount`}
        ...
        <Form.Control
          id={`otherResouce${id}CurrentAmount`}
          type="number"
          value={current}
