...
To update the database use this command from the Web project folder (replace `FinanceTrack.Finance` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../FinanceTrack.Finance.Infrastructure/FinanceTrack.Finance.Infrastructure.csproj -s FinanceTrack.Finance.Web.csproj
```
