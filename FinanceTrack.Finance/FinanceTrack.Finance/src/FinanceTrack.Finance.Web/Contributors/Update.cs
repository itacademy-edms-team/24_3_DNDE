using FinanceTrack.Finance.UseCases.Contributors.Get;
using FinanceTrack.Finance.UseCases.Contributors.Update;
    ...
    : Endpoint<UpdateContributorRequest, UpdateContributorResponse>
{
    ...
    )
    {
        var result = await _mediator.Send(
            new UpdateContributorCommand(request.Id, request.Name!),
            cancellationToken
        );
        ...
        if (queryResult.IsSuccess)
        {
            ...
            Response = new UpdateContributorResponse(
                new ContributorRecord(dto.Id, dto.Name, dto.PhoneNumber)
            );
            return;
        }
    }
