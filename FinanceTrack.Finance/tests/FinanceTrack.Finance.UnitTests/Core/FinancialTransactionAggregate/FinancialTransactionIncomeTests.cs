...
public class FinancialTransactionIncomeTests
{
    [Fact]
    public void CreateIncome_SetsExpectedFields()
    {
        ...
        var transaction = FinancialTransaction.CreateIncome(userId, name, amount, date, isMonthly);

        transaction.UserId.ShouldBe(userId);
        transaction.Name.ShouldBe(name);
        transaction.TransactionType.ShouldBe(FinancialTransactionType.Income);
    ...
    public void CreateIncome_InvalidAmount_ThrowsArgumentException(decimal badAmount)
    {
        ...
        Should.Throw<ArgumentException>(() =>
            FinancialTransaction.CreateIncome("user", "Salary", badAmount, date, false)
        );
    }
    ...
    {
        Should.Throw<ArgumentException>(() =>
            FinancialTransaction.CreateIncome("user", "Salary", 10, default, false)
        );
    }
    ...
    public void CreateIncome_RoundsAmountUp()
    {
        ...
        var transaction = FinancialTransaction.CreateIncome("user", "Salary", 10.999m, date, false);
    ...
    {
        var date = DateOnly.FromDateTime(DateTime.UtcNow.Date);
        var transaction = FinancialTransaction.CreateIncome("user", "Salary", 10m, date, false);
