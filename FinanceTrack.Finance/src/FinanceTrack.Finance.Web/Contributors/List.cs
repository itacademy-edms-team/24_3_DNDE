...
public class List(IMediator _mediator) : EndpointWithoutRequest<ContributorListResponse>
{
    ...
    {
        Result<IEnumerable<ContributorDTO>> result = await _mediator.Send(
            new ListContributorsQuery(null, null),
            cancellationToken
        );

        var result2 = await new ListContributorsQuery2(null, null).ExecuteAsync(cancellationToken);
        ...
        if (result.IsSuccess)
        {
            ...
            {
                Contributors = result
                    .Value.Select(c => new ContributorRecord(c.Id, c.Name, c.PhoneNumber))
                    .ToList(),
            };
        }
    }
