    ...
    : Endpoint<ListWalletTransactionsRequest, ListWalletTransactionsResponse>
{
    ...
    public override async Task HandleAsync(ListWalletTransactionsRequest req, CancellationToken ct)
    {
        ...
        var query = new ListWalletTransactionsQuery(userId, req.WalletId, req.Type, req.From, req.To);
        ...
        Response = new ListWalletTransactionsResponse
        {
            Transactions = result.Value
                .Select(t => new FinancialTransactionRecord(
                    t.Id, t.WalletId, t.Name, t.Amount, t.OperationDate,
                    t.Type, t.CategoryId, t.RelatedTransactionId, t.RecurringTransactionId
                ))
                .ToList(),
        };
    }
