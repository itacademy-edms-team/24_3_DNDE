...
Заполучите API-ключ репозитория, в который планируете публиковать пакет. Добавьте его в глобальный `Nuget.Config` в раздел `<apikeys>` по примеру ниже:
```xml
...
- `--skip-duplicate` позволяет не выдавать ошибку конфликта версий при попытке опубликовать уже хранящуюся в репозитории версию, а тихо завершить выполнение команды.
